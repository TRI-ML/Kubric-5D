...
CONTAINER_CARRIER_SPAWN_REGION = {
    'centered': [(-4, -4, 0), (4, 4, 4)],
    'scattered': [(-8, -8, 0), (8, 8, 8)]
}
STATIC_SPAWN_REGION = {
    'centered': [(-7, -7, 0), (7, 7, 7)],
    'scattered': [(-12, -12, 0), (12, 12, 8)]
}
DYNAMIC_SPAWN_REGION = {
    'centered': [(-5, -5, 1), (5, 5, 6)],
    'scattered': [(-9, -9, 1), (9, 9, 6)]
}
...
class MyKubricSimulatorRenderer:
    ...
    def __init__(self, frame_width=256, frame_height=192, num_frames=24, frame_rate=12,
        ...
        self.gso_source = kb.assets.AssetSource.from_manifest(
            'gs://kubric-public/assets/GSO/GSO.json')
        self.hdri_source = kb.assets.AssetSource.from_manifest(
            'gs://kubric-public/assets/HDRI_haven/HDRI_haven.json')
        self.kubasic_source = kb.assets.AssetSource.from_manifest(
    ...
    def setup_camera_trajectory(self, positions, look_ats, rolls=[0.], focal_lengths=[35.0]):
        start_time = time.time()

        num_waypoints = self.num_frames + 3
        if len(focal_lengths) == 1:
            focal_lengths = focal_lengths * num_waypoints
        if len(rolls) == 1:
            rolls = rolls * num_waypoints

        assert len(positions) == len(look_ats) == len(focal_lengths) == len(rolls) == num_waypoints, \
            "Number of waypoints should equal self.num_frames + 3"
        
        self.scene.camera = self.kb_module.PerspectiveCamera(
            focal_length=focal_lengths[0], sensor_width=32.0)

        for frame_idx in range(-1, num_waypoints - 1):
            position, look_at = positions[frame_idx + 1], look_ats[frame_idx + 1]
            focal_length, roll = focal_lengths[frame_idx + 1], rolls[frame_idx + 1]
            self.scene.camera.position = position
            self.scene.camera.look_at(look_at, roll)
            self.scene.camera.focal_length = focal_length
            self.scene.camera.keyframe_insert('position', frame_idx)
            self.scene.camera.keyframe_insert('quaternion', frame_idx)
            self.scene.camera.keyframe_insert('focal_length', frame_idx)
    ...
    def insert_static_objects(self, min_count=8, max_count=12, force_containers=0,
        ...
        for i, object_kind in enumerate(object_kinds):
            ...
            spawn_region = STATIC_SPAWN_REGION[placing] if object_kind == 'any' else \
                CONTAINER_CARRIER_SPAWN_REGION[placing] if object_kind in ['container', 'carrier'] else None
    ...
    def insert_dynamic_objects(self, min_count=4, max_count=6, force_boxes=0,
        ...
        for i, object_kind in enumerate(object_kinds):
            ...
            spawn_region = DYNAMIC_SPAWN_REGION[placing]
            self.move_until_no_overlap(
                obj, self.simulator, rotation_axis=rotation_axis, spawn_region=spawn_region,
                rng=self.random_state)
    ...
    def render_frames(self, frame_start, frame_end,
        ...
        data_stack = self.renderer.render(
            frame_start=frame_start, frame_end=frame_end, return_layers=return_layers)
