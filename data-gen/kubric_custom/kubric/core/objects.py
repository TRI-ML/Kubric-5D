...
def look_at_quat(
  ...
  if roll is not None:
    # Note (Dian): roll actually happens around the z axis so name the rot matrix "yaw"
    ...
    rotation_yaw = np.eye(3)
  rotation_matrix1 = np.stack([look_at_right, look_at_up, look_at_front])
  rotation_matrix2 = np.stack([right, up, front])
  return tuple(pyquat.Quaternion(matrix=(rotation_matrix1.T @ rotation_yaw @ rotation_matrix2)))
...
class Object3D(assets.Asset):
  ...
  def look_at(self, target, roll=None):
    self.quaternion = look_at_quat(self.position, target, self.up, self.front, roll)
