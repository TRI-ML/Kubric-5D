...
def add_png_output_node(base_path: str):
    """Add a PNG File Output node to the compositor for Filmic/sRGB RGBA."""
    tree = bpy.context.scene.node_tree
    links = tree.links

    # Find the Render Layers node that was just created
    render_node = tree.nodes.get("Render Layers")
    if render_node is None:
        raise RuntimeError("Render Layers node not found; did you run set_up_exr_output_node first?")

    # Create a new FileOutput node for PNG
    png_node = tree.nodes.new(type="CompositorNodeOutputFile")
    png_node.label = "PNG Output"
    png_node.base_path = base_path
    png_node.format.file_format = "PNG"
    png_node.format.color_mode = "RGBA"
    png_node.format.color_depth = "8"   # 8-bit PNGs
    png_node.file_slots[0].path = "frame_"

    # Connect Render Layers RGBA → PNG node
    links.new(render_node.outputs.get("Image"), png_node.inputs[0])
...
def apply_filmic(linear_rgba: np.ndarray) -> np.ndarray:
    """Convert linear float32 RGBA to Filmic sRGB uint8 RGBA."""
    config = ocio.GetCurrentConfig()
    # Use Blender's default roles: "scene_linear" → "srgb_display"
    processor = config.getProcessor("Linear", "Filmic sRGB")
    cpu_processor = processor.getDefaultCPUProcessor()

    h, w, c = linear_rgba.shape
    rgb = linear_rgba[..., :3].copy()
    cpu_processor.applyRGB(rgb)
    rgb_srgb = rgb.copy()

    # Keep alpha linear
    alpha = linear_rgba[..., 3:4]
    rgba = np.concatenate([rgb_srgb, alpha], axis=-1)
...
def get_render_layers_from_exr(filename) -> Dict[str, np.ndarray]:
  ...
  if "Image" in layer_names:
    ...
    output["rgba"] = apply_filmic(output["linear_rgba"])
